...
csv_file = 'SPID_VN.csv'  # Path where the output file (CSV report) will be saved
...
# Define the samples belonging to the SPID group
spid_samples = {'T-NGS_8192','T-NGS_8191','T-NGS_8188','T-NGS_8189','T-NGS_8190','T-NGS_8196','T-NGS_8197','T-NGS_8198',
...
print(f"Number of samples in SPID group: {len(spid_samples)}")
...
with open(csv_file, mode='w', newline='') as file:
    ...
    # Add SPID samples first, then VN samples
    for sample in samples:
        if sample in spid_samples:
            group = "SPID"
            header += [f"{group}_{sample} GT", f"{group}_{sample} DP", f"{group}_{sample} GQ", f"{group}_{sample} AD"]
    
    # Add VN samples after SPID samples
    for sample in samples:
    ...
    for record in reader:
        ...
        # Process SPID samples first, then VN samples
        for sample_name in samples:
            if sample_name in spid_samples:
                group = "SPID"
        ...
        # Process VN samples after SPID samples
        for sample_name in samples:
